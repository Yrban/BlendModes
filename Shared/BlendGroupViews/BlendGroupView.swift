...
struct BlendGroupView: View {
    ...
    @State private var minDim: CGFloat = 300
    
    private var segments: [LayerSegment] {
        buildSegments()
    }
    
    private var bgColor: Color {
        blendModel.layers.last(where: { $0.type == .background })?.color ?? Color.gray
    }
    ...
    private func imagesCanvas(layer: Layer) -> some View {
        ZStack {
            if let data = layer.imageData, let image = platformImage(from: data) {
                image
                    .resizable()
                    .scaledToFit()
            } else {
